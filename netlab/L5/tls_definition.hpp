...
using SessionID = std::array<uint8_t, 32>; /* Represents SessionID session_id<0..32>; */
using CipherSuite = std::array<uint8_t, 2>; /* Represents CipherSuite cipher_suites<2..2^16-1>; */
...
using DistinguishedName = std::vector<uint8_t>; /* Represents DistinguishedName<1..2^16-1>; */
...
/************************** DONT KNOW IF NEEDED *************************/
struct TLSHeader {
	ContentType type; /* Content type (ContentType). */
	ProtocolVersion version; /* TLS version (ProtocolVersion). */
...
struct TLSSession {
	...
	X509* peer_cert; /* peer certificate. */
	PRFAlgorithm prf_algorithm;
	BulkCipherAlgorithm bulk_cipher_algorithm;
	CipherType cipher_type;
	MACAlgorithm mac_algorithm;
